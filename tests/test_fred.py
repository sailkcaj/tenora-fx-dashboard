...
def test_fetch_passes_window_and_parses():
    ...
    assert len(df) == 3
    assert [c["params"] for c in session.calls] == [{"id": "DGS2", "cosd": "2024-09-05", "coed": "2026-09-05"}]
    assert session.calls[0]["headers"]["User-Agent"].startswith("tenora-fx-dashboard")
