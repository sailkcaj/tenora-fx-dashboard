...
def rates_ok(fred_id, start, end, session=None):
...
def test_pull_fx_fallback_success_is_noted(tmp_path):
    pairs = (FxPair("USDCNH", "USDCNH=X", fallbacks=("CNH=X",)),)
    ...
    assert result.source == "yfinance:CNH=X"
    assert "fallback" in result.note


def test_pull_rates_marks_unavailable_and_failed(tmp_path):
    series = (
        RateSeries("us_2y", "US", "2y_yield", "DGS2", "US 2y"),
        RateSeries("uk_2y", "UK", "2y_yield", None, "UK 2y", note="candidate source: BoE"),
        RateSeries("jp_policy_rate", "JP", "policy_rate", "BAD", "JP policy"),
    )

    def fake_fetch(fred_id, start, end, session=None):
        if fred_id == "BAD":
            raise FredError("BAD: HTTP 404, series not found on FRED")
        return rate_frame(24)

    results = pull_rates(series, START, END, rates_dir=tmp_path, fetch=fake_fetch, pause=0)
    assert {r.key: r.status for r in results} == {"us_2y": OK, "uk_2y": UNAVAILABLE, "jp_policy_rate": FAILED}
    assert (tmp_path / "us_2y.parquet").exists()
    assert not (tmp_path / "jp_policy_rate.parquet").exists()
    uk = next(r for r in results if r.key == "uk_2y")
    assert uk.error == "not available on FRED"
    assert uk.note == "candidate source: BoE"
...
def test_run_writes_manifest_and_report(tmp_path, capsys):
    ...
    assert "=== rates/uk_2y  [fred:none]  UNAVAILABLE: not available on FRED" in out
    assert out.count("\n2026-") == 4  # two tail rows for each of the two files
    assert out.strip().splitlines()[-1] == (
        "SUMMARY: 2/3 series ok | FX 1/1 | rates 1/2 | NOT OK: uk_2y [unavailable: not available on FRED]"
    )
...
def test_partial_run_keeps_previous_manifest_entries(tmp_path):
    common = dict(
        pairs=(FxPair("EURUSD", "EURUSD=X"),),
        series=(RateSeries("us_2y", "US", "2y_yield", "DGS2", "US 2y"),),
...
def test_main_exit_code_reflects_failures(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pipeline, "run", lambda **kwargs: [
        PullResult("rates", "uk_2y", "fred:none", UNAVAILABLE, error="not available on FRED"),
    ])
